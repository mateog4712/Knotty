...
TraceArrows::TraceArrows(cand_pos_t n, const MType &src_mtype, const std::vector<int> *index)
    : n_(n),
      index_(index),
      ta_count_(0),
      ta_avoid_(0),
...
cand_pos_t TraceArrows::number() const {
    cand_pos_t c=0;
    for ( auto &x: trace_arrow_ ) {
        c += x.size();
    }
...
cand_pos_t TraceArrows::capacity() const {
    cand_pos_t c=0;
    for ( auto &x: trace_arrow_ ) {
        c += x.capacity();
    }
...
void MasterTraceArrows::register_trace_arrow(const Index4D &src_x,
                                        const Index4D &tgt_x,
                                        const MType &mtype,
                                        energy_t e) {
 ...
 */
void
MasterTraceArrows::gc_row( cand_pos_t i, TraceArrows &source ) {
 ...
 */
void
MasterTraceArrows::gc_to_target(cand_pos_t i, cand_pos_t j, cand_pos_t k, cand_pos_t l, TraceArrows &target) {
    if (ta_debug)
...
MasterTraceArrows::MasterTraceArrows(cand_pos_t n, const std::vector<int> &index)
    : n_(n),
      index_(&index),
      PL(n,MType::L,&index),
      PR(n,MType::R,&index),
...
void
MasterTraceArrows::garbage_collect(cand_pos_t i) {
#ifndef TEMP_DEACTIVATE_GC
    gc_row(i, PL);
    gc_row(i, PR);
    gc_row(i, PM);
