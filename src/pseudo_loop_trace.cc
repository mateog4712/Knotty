...
void pseudo_loop::recompute_slice_PXdecomp(const Index4D &x,int decomp_cases,candidate_lists &CL,const TriangleMatrix &w,const MatrixSlices3D &PXsrc,MatrixSlices3D &PXtgt){
    cand_pos_t i     = x.i();
    cand_pos_t max_j = x.j();
    cand_pos_t min_k = x.k();
    cand_pos_t max_l = x.l();

    for (cand_pos_t l=min_k; l<=max_l; l++) {
        for (cand_pos_t k=l; k>=min_k; k--) {
            for (cand_pos_t j=i; j<=max_j; j++) {
                ...
                if ((energy = CL.find_candidate(i,j,k,l)) < INF/2) {
                    ...
                    energy_t min_energy = generic_decomposition(i, j, k, l, decomp_cases, CL, w, PXsrc);
                    PXtgt.set(i, j, k, l, min_energy);
                }
            }
        }
    }
...
energy_t pseudo_loop::recompute_PX(const Index4D &x, MType type) {
    ...
    // from case
    temp = calc_PfromX(x, type) + penalty(x,gamma2,type);
    if (temp < min_energy) {
        min_energy = temp;
